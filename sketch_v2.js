...
function draw() {
  ...
  image(leaves, 0,0, windowWidth, windowHeight)
    ...
    if (millis() - lastLongTime > longInterval) {
        ...
        // Check if the motion type has changed
    if (currentMotionType !== previousMotionType) {
        // Update previousMotionType
        previousMotionType = currentMotionType;

        // Check for specific motion types
        if (currentMotionType == "circle") {
            // Perform the circle-related interaction
            console.log("circle")
        }
        else if (currentMotionType == "wave"){
            console.log("wave")
            delay = new p5.Delay();
...
function fetchMotionType() {
    const motionTypeUrl = 'https://api.particle.io/v1/devices/kat_cat/motionType?access_token=d0b2c1391dec30c25940308c6c1fda58395359be';
